...
**4. Pseudo Labeling (soft labels)**
- Предсказываем test (250k) → добавляем как soft labels к train → 1M
- Soft labels (вероятности) лучше hard (0/1) — регуляризация
- K-fold pseudo labels чтобы избежать утечки
...
**6. colsample_bytree=0.05 (агрессивный feature sampling)**
- При 2440 фичах каждое дерево видит ~120 — сильная регуляризация
- Работает именно на обфусцированных данных с большим числом фичей
...
Мы на правильном пути: per-target XGBoost + Optuna + стекинг = **LB 0.8472**. Но победители используют **3-7 разных моделей в L1**, а мы только 1. Путь к 0.86 — через разнообразие моделей (CatBoost со своим FS) + pseudo labeling + Optuna на большей выборке для слабых таргетов.
